...
## byrdOS Implementation Notes

> Applies the Notion design language to the byrdOS stack. Consult `AGENTS.md` (FRONTEND WORKFLOW section) for the 8-phase pipeline.

- **Framework**: Tailwind CSS v4 (`@theme` tokens in `apps/web/src/app/globals.css`) + shadcn/ui (Radix primitives in `packages/ui/src/components`).
- **Font**: `NotionInter` -> **Inter** via `next/font` (the spec's own substitute note); apply the negative letter-spacing values explicitly to approximate the tight display tracking.
- **Token mapping**: map the front-matter `colors`, `typography`, `spacing`, and `rounded` tokens into Tailwind v4 `@theme` (semantic names: `primary`, `primary-active`, `secondary`, `canvas`, `canvas-soft`, `surface`, `ink`, `ink-secondary`, `ink-muted`, `ink-faint`, `hairline`, `accent-*`).
- **Components**: restyle shadcn/ui primitives (Button, Card, Input, Badge, Table, Dialog, Toast, etc.) to the spec -- pill `rounded-full` primary CTAs, `rounded-md` utility buttons, `rounded-lg` cards with `hairline` border + Level-1 micro-shadow, `rounded-xs` inputs.
- **Elevation**: hairline + layered micro-shadows (Level 0/1/2) -- never heavy drop-shadows.
- **Accessibility**: WCAG 2.2 AA (>= 4.5:1 text contrast, 44x44px touch targets, `focus-visible` rings, `prefers-reduced-motion`).
- **Motion**: subtle, purposeful, `motion-safe:` wrapped; no auto-play.
- **State indicators**: syncing/error/empty/offline per AGENTS.md (skeletons over spinners, EmptyState, ErrorBoundary, Sonner toasts).
- **Dark mode**: daylight-first spec; dark mode inverts canvas/surface/ink while keeping the single blue accent + sticker palette.
