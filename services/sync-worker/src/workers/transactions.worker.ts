...
export function createTransactionsWorker(): Worker<TransactionsJobData> {
  const worker = new Worker<TransactionsJobData>(
    ...
    async (job) => {
      ...
      const span = getTracer().startSpan('sync.transactions', {
        parentContext,
        attributes: {
          syncJobId,
      ...
      try {
        ...
        for await (const batch of iterable) {
          ...
          addedCount += batch.added.length;
          modifiedCount += batch.modified.length;
          removedCount += batch.removed.length;
        ...
        span.setAttribute('transaction.added', addedCount);
        span.setAttribute('transaction.modified', modifiedCount);
        span.setAttribute('transaction.removed', removedCount);
        if (finalCursor !== null) {
          span.setAttribute('nextCursor', finalCursor);
        }
        span.setStatus('OK');
